...
1.	Compares the running code version to the currently compiled code
2.	Compares active settings to production/SDS/class settings
3.	If, and only if (1) or (2) show a mismatch, the Business Host would be marked as out-of-date and requiring an update.
...
This way, “updating” the code or settings without changing anything results in no production downtime.
...
Note, that if you update a production using [produciton deployment file](https://docs.intersystems.com/irislatest/csp/docbook/DocBook.UI.Page.cls?KEY=EGDV_deploying) it would atomatically disable/enable all affected BHs.
