Продолжая [предыдущую статью](https://habr.com/ru/company/intersystems/blog/345532/) о разработке REST API на платформе InterSystems IRIS, в этой статье я хотел бы рассказать о подходе от спецификации (spec-first) при разработке REST API на платформе InterSystems IRIS.
