...
Конфигурация платформы данных InterSystems IRIS во многом определяется файлом `iris.cpf`. При каждом запуске InterSystems IRIS, считывается этот конфигурационный файл (далее CPF), для получения значений параметров работы инстанса. Это позволяет в любой момент изменить конфигурацию инстанса, изменив его CPF и перезапустив InterSystems IRIS. 
